...
The XFI AI service must remain unprivileged. This helper is the only privileged
boundary and accepts JSON actions on stdin. It never accepts arbitrary paths,
services, or shell commands.
"""
...
PROJECTS = {
    "connect": {
        "path": Path("/root/XFI_CONNECT"),
        "service": "xfi-connect",
        "health_env": "XFI_CONNECT_HEALTH_URL",
        "default_health": "",
    },
    "webapp": {
        "path": Path("/opt/XFI_3XUI_WebApp"),
        "service": "xfi-3xui-webapp",
        "health_env": "XFI_3XUI_WEBAPP_HEALTH_URL",
        "default_health": "http://127.0.0.1:3000/health",
    },
}
...
def validate(root: Path, project: str, changed: list[str]) -> None:
    if project == "connect":
        for rel in changed:
            if rel.endswith(".py"):
                subprocess.run(["python3", "-m", "py_compile", rel], cwd=root, check=True, timeout=30, stdout=subprocess.DEVNULL, stderr=subprocess.PIPE)  # nosec B603 - fixed argv
    else:
        subprocess.run(["bun", "run", "typecheck"], cwd=root, check=True, timeout=90, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True)  # nosec B603 - fixed argv


def restart(service: str) -> None:
    subprocess.run(["systemctl", "restart", service], check=True, timeout=30, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True)  # nosec B603 - fixed argv
    result = subprocess.run(["systemctl", "is-active", service], check=False, timeout=10, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True)  # nosec B603 - fixed argv
