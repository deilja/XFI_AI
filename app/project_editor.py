...
PROJECTS = {
    "connect": {
        "name": "XFI_CONNECT",
        "path_env": "XFI_CONNECT_PATH",
        "default_path": "/root/XFI_CONNECT",
        "service_env": "XFI_CONNECT_SERVICE",
        "default_service": "xfi-connect",
        "kind": "python",
    ...
    "webapp": {
        ...
        "path_env": "XFI_3XUI_WEBAPP_PATH",
        "default_path": "/opt/xfi-3xui-webapp",
        "service_env": "XFI_3XUI_WEBAPP_SERVICE",
        "default_service": "xfi-3xui-webapp",
        "kind": "bun",
...
SYSTEM_PROMPT = """Ты — инженер XFI AI. Ты работаешь только с выбранным установленным проектом XFI.
Не придумывай файлы/API и не смешивай проекты. Не читай и не меняй секреты.
Для анализа верни только JSON: {\"ready\":true/false,\"questions\":[...],\"summary\":\"...\",\"files\":[\"path\"]}.
Если данных недостаточно, ready=false и максимум 3 конкретных вопроса.
"""

PATCH_PROMPT = """Сформируй минимальные изменения только для выбранного установленного проекта.
Верни только JSON: {\"summary\":\"...\",\"edits\":[{\"path\":\"...\",\"content\":\"полное новое содержимое\",\"reason\":\"...\"}],\"tests\":[\"...\"]}.
Только существующие безопасные файлы из контекста. Не трогай .env, secrets, credentials, private keys, сертификаты. Максимум 8 файлов. Не меняй другой проект.
"""
